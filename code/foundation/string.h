 ...
 *  character which we can use for null-termination. 
 ...
 * */
template <class T>
struct str_alloc_wrapper
{
    using value_type = memory::_allocator_value<T>;
    using propagate_on_container_copy_assignment = memory::_pocca<T>;
    using propagate_on_container_move_assignment = memory::_pocma<T>;
    using propagate_on_container_swap = memory::_pocs<T>;
    using is_always_equal = memory::_is_always_equal<T>;
    
    template <class X>
    struct rebind {
        using type = str_alloc_wrapper<memory::_rebind<T, X>>;
    };
    
    str_alloc_wrapper() = default;
    str_alloc_wrapper(str_alloc_wrapper &) = default;
    str_alloc_wrapper(const str_alloc_wrapper &) = default;
    str_alloc_wrapper(str_alloc_wrapper &&) = default;
    
    template <class X>
    inline str_alloc_wrapper(str_alloc_wrapper<X> &x)
    : alloc(x.alloc)
    {}
    template <class X>
    inline str_alloc_wrapper(const str_alloc_wrapper<X> &x)
    : alloc(x.alloc)
    {}
    template <class X>
    inline str_alloc_wrapper(str_alloc_wrapper<X> &&x)
    : alloc(foundation::move(x.alloc))
    {}
    
    str_alloc_wrapper &operator = (str_alloc_wrapper &) = default;
    str_alloc_wrapper &operator = (const str_alloc_wrapper &) = default;
    str_alloc_wrapper &operator = (str_alloc_wrapper &&) = default;
    
    inline str_alloc_wrapper select_on_container_copy_construction()
    {
        return str_alloc_wrapper{memory::socc(alloc)};
    }
    ...
    {
        value_type *res = memory::allocate(alloc, n+1, h);
        res[n] = 0;
        return res;
    }
    ...
    {
        return memory::reallocate(alloc, ptr, old_size+1, new_size+1);
    }
    inline void deallocate(value_type *ptr, const size_t n) noexcept {
        memory::deallocate(alloc, ptr, n+1);
    }
    
    template <class ... X>
    inline void construct(value_type *ptr, X && ... x)
    {
        memory::construct(alloc, ptr, forward<X>(x)...);
    }
    inline void destroy(value_type *ptr)
    {
        memory::destroy(alloc, ptr);
    }
    
    inline bool operator == (const str_alloc_wrapper &other) const noexcept {
...
class basic_string
{    
    using allocator_type_ = string_detail::str_alloc_wrapper<Allocator>;
...
public:
    ...
    noexcept(noexcept(
        declval<container_type_&>().swap(declval<container_type_&>()) ))
    {
        v.swap(other.v);
    }
