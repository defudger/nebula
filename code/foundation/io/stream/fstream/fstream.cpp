    ...
    const io_detail::mode m, const bool trunc)
{
    ...
    case io_detail::mode::in:
        ...
        if(!h)
            n_throw(io_error, system::system_error)
            << ei_msg_c("fopen() failed.")
            << ei_msg(string("file: ") + filename)
    ...
    case io_detail::mode::out:
        ...
        else
        {
            ...
            if(!h)
                ...
                << ei_msg(string("file: ") + filename)
                << system::ei_errno_code();
            if(!freopen(filename.c_str(), "r+b", h)) {
                this->close();
                n_throw(io_error, system::system_error)
                << ei_msg_c("freopen() failed.")
                << ei_msg(string("file: ") + filename)
                << system::ei_errno_code();
            }
        }
    ...
    case io_detail::mode::in_out:
        if(trunc)
        {
            ...
            if(!h)
                ...
                << ei_msg(string("file: ") + filename)
                << system::ei_errno_code();
            if(!freopen(filename.c_str(), "a+b", h)) {
                this->close();
                n_throw(io_error, system::system_error)
                << ei_msg_c("freopen() failed.")
                << ei_msg(string("file: ") + filename)
    ...
    case io_detail::mode::out:
        if(trunc)
        {
            ...
            if(!h)
                ...
                << ei_msg(string("file: ") + filename)
                << system::ei_errno_code();
            if(!freopen(filename.c_str(), "ab", h)) {
                this->close();
                n_throw(io_error, system::system_error)
                << ei_msg_c("freopen() failed.")
                << ei_msg(string("file: ") + filename)
