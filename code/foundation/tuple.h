...
>;
//------------------------------------------------------------------------------
template <class N, class I = meta::size_t_<0>, class Seq = meta::seq<>>
struct gen_index_seq_n_
{
    using type = typename meta::_if<
        meta::not_equal_<I, N>,
        gen_index_seq_n_<
            N,
            meta::_add<I, meta::size_t_<1>>,
            meta::_push_back<Seq, I>>,
        Seq>::type;
};
//------------------------------------------------------------------------------
template <class N>
using _gen_index_seq_n = typename gen_index_seq_n_<N>::type;
//------------------------------------------------------------------------------
...
struct tuple_element_ {
    using type = typename tuple_element<N::type::value, T>::type;
};
template <class N, class T>
using _tuple_element = typename tuple_element_<N, T>::type;

//------------------------------------------------------------------------------
template <class T>
struct tuple_size : public T::tuple_size {};

n_meta_alias(tuple_size);
...
//------------------------------------------------------------------------------
template <class Tuple_, class ... I>
struct tuple_expander<Tuple_, meta::seq<I...>>
{
    template <class F, class Tuple>
    inline static auto apply(F &&f, const Tuple &t)
    {
        return f(foundation::get<I::value>(t)...);
    }
    template <class F, class Tuple>
    inline static auto apply(F &&f, Tuple &t)
    {
        return f(foundation::get<I::value>(t)...);
    }
    template <class F, class Tuple>
    inline static auto apply(F &&f, Tuple &&t)
    {
        return f(
            foundation::forward<
                typename tuple_element<I::value, ctraits::_value<Tuple>>::type
            >(
...
//------------------------------------------------------------------------------
template <class F, class Tuple>
inline auto expand_tuple(F &&f, Tuple &&t)
{
    return tuple_detail::tuple_expander<
        ctraits::_value<Tuple>,
        tuple_detail::_gen_index_seq_n<_tuple_size<ctraits::_value<Tuple>>>
        >::apply(f, foundation::forward<Tuple>(t));
}
