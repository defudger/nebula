...
public:
    ...
    inline void operator () (Stream &s, fmt::fout_tag) const
    {
        ...
        case net::address_family::inet6:
            ...
            break;
        case net::address_family::local:
            fmt::fwrite(s, variant_cast<local::endpoint>(v));
            break;
        default:
            break;
        }
...
template <>
struct dispatch_address_family<address_family::local> {
    using address_type = local::endpoint;
    using endpoint_type = local::endpoint;
};
