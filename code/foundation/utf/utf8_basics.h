...
struct widen_impl
{
    inline static optional<char32_t> apply(const Range &r)
    {
 ...
 * */
template <class Range>
inline optional<char32_t> widen(const Range &r, nothrow_tag)
{
...
{
    auto res = utf::widen(r, nothrow_tag());
    if(!res.valid())
        n_throw(utf_error);
    return res.get();
}
 ...
 * */
template <class Sink>
inline bool narrow(const char32_t c, Sink &&sink, nothrow_tag)
{
    const size_t n = utf_detail::u32_to_u8_codeunit_count(c);
    if(0 == n)
        return false;
    utf_detail::u32_to_u8(c, n, sink);
    return true;
}
//------------------------------------------------------------------------------
template <class Sink>
inline bool narrow(const char32_t c, Sink &&sink)
{
    if(!narrow(c, sink, nothrow_tag()))
        n_throw(utf_error);
}
//------------------------------------------------------------------------------
 ...
 * */
inline const_cstring narrow(const char32_t c, nothrow_tag)
{
    thread_local static char buf[5] = {};
    char *i = buf;
    if(!utf::narrow(c,
        [&] (const char x) noexcept { *i++ = x; },
        nothrow_tag()))
    {
        return const_cstring{};
    }
...
{
    const const_cstring r = utf::narrow(c, nothrow_tag());
    if(r.empty())
        n_throw(utf_error);
    return r;
}
