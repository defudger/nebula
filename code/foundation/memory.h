 ...
 *
 *  If two allocators compare equal then memory allocated by one of them can
 ...
 *  true_type.
 *
 *  Move-construction and move-assignment of allocators must effectively
 *  transfer resource ownership. If resources acquired by allocator A
 *  then B = move(A) must enable to manage resources of A.
 *  Allocators must also swap resource ownership if they are swapped using
 *  swap(). Two allocators compare equal if one has been copy
 *  constructed/assigned from the other.
 *
 ...
 *  that basically returns a copy of the internal allocator.
 *  If at all available, then this member shall not throw.
 *
 *  pocca == true_type then the allocator must be copied by copy-assignment
 *  if the container is copy-assigned - even if the container is empty!
 ...
 *  if the container is move-assigned - even if the container is empty!
