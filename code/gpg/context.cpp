...
//------------------------------------------------------------------------------
errc context::encrypt(const key &key,
    io::stream_ref<io::in_tag, io::in_seek_tag> in,
    io::stream_ref<io::out_tag, io::out_seek_tag> out,
    const bool _ascii_armor)
{
    ...
    vector<gpgme_key_t> native_keys;
    native_keys.emplace_back(key.native_handle());
    native_keys.emplace_back(nullptr);
...
//------------------------------------------------------------------------------
errc context::sign(const key &key,
    io::stream_ref<io::in_tag, io::in_seek_tag> in,
    io::stream_ref<io::out_tag, io::out_seek_tag> out,
    const bool _ascii_armor)
{
    ...
    gpgme_error_t err = gpgme_signers_add(ctx, key.native_handle());
    if(GPG_ERR_NO_ERROR != err)
        n_throw(gpg::gpg_error)
        << ei_msg_c("gpgme_signers_add() failed")
        << ei_gpgme_error(err);
